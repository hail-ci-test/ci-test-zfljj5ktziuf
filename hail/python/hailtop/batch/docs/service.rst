...
`<https://auth.hail.is/user>`__. If you want to run gcloud or gsutil commands within your code, the service
...
.. code-block:: python
    ...
    ...     billing_project='my-billing-account', bucket='my-bucket') # doctest: +SKIP
...
You may elide the ``bucket`` parameter if you have previously set a
bucket with ``hailctl``:
